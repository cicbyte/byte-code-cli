...
**身份纪律**：`register` 对已有凭证的 profile 默认拒绝覆盖（显示新旧身份与实例差异，`--force` 才放行）；凭证记录注册实例 server_url，`profiles` 输出 agent_id 与实例归属列（多实例防串的关键自查面）。

**任务类型纪律**：`bcode create` 务必带 `--type`——bug 修复 `--type bug`、清理/杂务 `--type chore`、测试 `--type test`、新功能 `--type feature`（缺省 feature 会让类型统计与来源列失真）。
